...
namespace FroniusApp.Commands
{
    ...
    public class AppCommand : BaseRootCommand
    {
        ...
        public AppCommand(IConfiguration configuration,
                          FroniusGateway gateway,
                          GlobalOptions options,
                          InfoCommand infoCommand,
                          ReadCommand readCommand,
                          MonitorCommand monitorCommand,
                          ILogger<AppCommand> logger)
            : base(options, logger, "Allows to read data from a Fronius Symo 8.2-3-M solar inverter.")
        {
            ...
            AddGlobalOption(new Option<string>(
                alias: "--deviceid",
                description: "Global Fronius device id.")
                .Default(options.DeviceID)
                .Name("ID")
            );
            ...
            Handler = CommandHandler.Create<IConsole, GlobalOptions>((console, options) =>
                {
                    ...
                    if (options.Verbose)
                    {
                        ...
                        console.Out.WriteLine($"Device ID:     {options.DeviceID}");
                    ...
                    {
                        Console.WriteLine($"Fronius web service with device ID '{options.DeviceID}' found at {options.Address}.");
                    }
                    else
                    {
                        Console.WriteLine($"Fronius web service with device ID '{options.DeviceID}' not found at {options.Address}.");
                    }
